...
public final class Sweep {
    ...
    /**
     * Ray cast from point (ox, oy) with direction {@code dir} against an AABB.
     * Returns {@link SweptResult#NONE} if the ray misses or the hit is beyond t=1.
     */
    static SweptResult rayVsAABB(float ox, float oy, Vec2 dir, AABB box) {
        float tMinX, tMaxX, tMinY, tMaxY;

        if (Math.abs(dir.x) < 1e-6f) {
            // Ray is parallel to Y-axis — no intersection on X slab unless inside
        ...
        // Miss: slabs don't overlap, or intersection is behind ray, or beyond step
        if (tExit <= tEnter || tEnter >= 1f || tExit <= 0f) return SweptResult.NONE;

        float t = Math.max(tEnter, 0f);

        // Determine which face was hit to compute the normal
        Vec2 normal;
        if (tMinX > tMinY) {
            // Hit X slab first
    ...
    private static Vec2 computePushOutNormal(AABB a, AABB b) {
        // Find the axis with smallest overlap and push out along it
